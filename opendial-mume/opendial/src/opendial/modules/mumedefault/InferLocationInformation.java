...
import static opendial.modules.mumedefault.CarPoolingInformationExtraction.*;
import static opendial.modules.mumedefault.config.Shared.*;
import static opendial.modules.mumedefault.information.Slot.SLOT_ID_SEPARATOR;
...
class InferLocationInformation {
    static void inferMissingInfo(Map<String, String> oldInformation,
        ...
        */
        ...
        if (city != null)   // 3 - 4
            newInformation.put((isStart) ? START_CITY : END_CITY, city.getName());
        else if (slot != null) {
            newInformation.put((isStart) ? START_CITY : END_CITY, slot.getCity().getName());
            inferredCity = true;
        }
